// This is the best nCr we can find out
// Time Complexity is O(log^2(n))
// We have used Chinese Remainder theorem for finding nCr mod m
// For finding (n!)p mod p^e, we have used Andrew Granvelli paper
...
ll chineseRemainder(int k, ll M){								// now we have k equations
	ll ans=0, ai, ri;
	// ll M=1;									// Note: M is itself m (input m)
	//loop(i,k)
